...
#>
function lint() {
    poetry run flake8 src\boxx_pymonad tests
}
...
#>
function coverage() {
    poetry run coverage run --source src\boxx_pymonad -m pytest
...
#>
function docs(){
    if (Test-Path ".\docs\boxx_pymonad.rst") { Remove-Item ".\docs\boxx_pymonad.rst" -Force }
    if (Test-Path ".\docs\modules.rst") { Remove-Item ".\docs\modules.rst" -Force }
    poetry run sphinx-apidoc -o docs\ src\boxx_pymonad
